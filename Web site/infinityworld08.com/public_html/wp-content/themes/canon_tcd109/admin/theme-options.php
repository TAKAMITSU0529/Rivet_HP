...
<div class="wrap">
 ...
 <h2><?php _e( 'TCD Theme Options', 'tcd-canon' ); ?></h2> 
 ...
 <div class="updated fade">
  <p><strong><?php _e( 'Updated', 'tcd-canon' ); ?></strong></p>
 </div>
 <?php endif; ?>
 ...
 <div id="my_theme_option" class="cf">
  ...
  <div id="my_theme_right">
   ...
   <div id="saving_data" style="display:none;"><p><?php _e('Now saving...', 'tcd-canon'); ?></p></div>
  </div>
...
function theme_options_tools_notices() {
	...
	if ( isset( $tools_result[0], $tools_result[1] ) && 'import' === $tools_result[0] && 'error' === $tools_result[1] ) {
		...
		if ( isset( $tools_result[2] ) ) {
			...
			switch( $int_import_error ) {
				...
				case 8:
					...
					break;
				case 11:
                  $message = $int_import_error . ' : ' . __( 'File has not been uploaded.', 'tcd-canon' );
					break;
				case 12:
                  $message = $int_import_error . ' : ' . __( 'There is no file', 'tcd-canon' );
					break;
				case 13:
                  $message = $int_import_error . ' : ' . __( 'The file extension is not .json', 'tcd-canon' );
					break;
				case 14:
                  $message = $int_import_error . ' : ' . __( 'Failed to read the file', 'tcd-canon' );
					break;
				case 15:
                    $message = $int_import_error . ' : ' . __( 'Json decoding failed', 'tcd-canon' );
					break;
				default :
		...
		echo '<div class="update-message notice notice-error notice-alt"><p><strong>' . sprintf( __( 'Import error. %s', 'tcd-canon' ), $message ) . '</strong></p></div>';

	// インポート成功メッセージ
	} elseif ( isset( $tools_result[0], $tools_result[1] ) && 'import' === $tools_result[0] && 'success' === $tools_result[1] ) {
        echo '<div class="updated"><p><strong>' . __( 'Settings imported', 'tcd-canon' ) . '</strong></p></div>';

	// リセット成功メッセージ
	} elseif ( isset( $tools_result[0], $tools_result[1] ) && 'reset' === $tools_result[0] && 'success' === $tools_result[1] ) {
        echo '<div class="updated"><p><strong>' . __( 'Settings reset', 'tcd-canon' ) . '</strong></p></div>';
	}
}
...
/**
 * canon オプションTools デフォルト画像設定フィルター
 */
function canon_theme_options_tools_get_default_images_settings( $default_images_settings ) {
	// デフォルト画像設定
	return array(
/*
		array(
...
*/
		...
		array(
			'media_filename' => 'image_1450x800.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_1450x800.gif',
			'theme_option_keys' => array("index_slider")
		),
		array(
			'media_filename' => 'image_645x600.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_645x600.gif',
			'theme_option_keys' => array("['contents_builder'][0]['image_slider']")
		),
		array(
			'media_filename' => 'image_330x520.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_330x520.gif',
			'theme_option_keys' => array("['contents_builder'][1]['item_list'][0]['image']","['contents_builder'][1]['item_list'][1]['image']","['contents_builder'][1]['item_list'][2]['image']")
		),
		array(
			'media_filename' => 'image_500x350.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_500x350.gif',
			'theme_option_keys' => array("['contents_builder'][3]['item_list'][0]['image']","['contents_builder'][3]['item_list'][1]['image']")
		),
		array(
			'media_filename' => 'image_362x180.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_362x180.gif',
			'theme_option_keys' => array("['footer_banner_list'][0]['image']","['footer_banner_list'][1]['image']","['footer_banner_list'][2]['image']","['footer_banner_list'][3]['image']")
		),
		array(
			'media_filename' => 'image_1450x560.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_1450x560.gif',
			'theme_option_keys' => array("archive_blog_header_image","archive_news_header_image","archive_gallery_header_image","archive_service_header_image")
		),
	);
}
...
/**
 * canon オプションTools サンプルカテゴリー
 */
function canon_theme_options_tools_set_sample_categories( $dp_options ) {
  ...
  // お知らせカテゴリー
	for ( $i = 1; $i <= 3; $i++ ) {
		$sample_categories[] = array(
			...
			'description' => __( 'Description will be displayed here.', 'tcd-canon' ),
		);
	}
...
/**
 * canon オプションTools サンプル記事 ---------------------------------------------------------------------------
 */
function canon_theme_options_tools_set_sample_posts( $dp_options ) {

	// ブログのアイキャッチ画像
	$media_id = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'image_960x580.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_960x580.gif'
	), true );

	// お知らせのアイキャッチ画像
	$news_media_id = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'image_710x460.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_710x460.gif'
	), true );

	// 客室のアイキャッチ画像
	$gallery_media_id = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'image_1290x640.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_1290x640.gif'
	), true );

	// サービスのアイキャッチ画像
	$service_media_id = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'image_1450x560_2.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_1450x560_2.gif'
	), true );
	...
	for ( $i = 1; $i <= 6; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample post', 'tcd-canon' ) . $i,
			'post_content' => __( 'Sample text. Sample text.', 'tcd-canon' ),
	...
	for ( $i = 1; $i <= 4; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample news', 'tcd-canon' ) . $i,
			'post_content' => __( 'Sample text. Sample text.', 'tcd-canon' ),
	...
	for ( $i = 1; $i <= 4; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample gallery', 'tcd-canon' ) . $i,
			'post_content' => __( 'Sample text. Sample text.', 'tcd-canon' ),
			...
			'taxonomies' => array(
				'gallery_category' => array('gallery_category1','gallery_category2','gallery_category3')
			),
			'metas' => array(
				'gallery_catch' => __( 'Catchphrase will be displayed here.', 'tcd-canon' ),
			)
		);
	}
	...
	for ( $i = 1; $i <= 4; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample service', 'tcd-canon' ) . $i,
			'post_content' => __( 'Sample text. Sample text.', 'tcd-canon' ),
			'post_name' => 'sample-style' . $i,
			'post_status' => 'publish',
			'post_type' => 'service',
			...
			'metas' => array(
				'service_sub_title' => __('Sub title', 'tcd-canon'),
			)
		);
	}
...
/**
 * canon オプションTools サンプルメニュー ------------------------------------------------------------------
 */
function canon_theme_options_tools_set_sample_menus( $dp_options ) {
	...
	if ( $nav_menus && ! is_wp_error( $nav_menus ) ) {
		foreach( $nav_menus as $nav_menu ) {
			if ( $nav_menu->name == __( 'Sample menu', 'tcd-canon' ) )
				return $dp_options;
		}
	}

	// サンプルメニュー作成
	$menu_id = wp_create_nav_menu( __( 'Sample menu', 'tcd-canon' ) );
...
/**
 * canon オプションTools サンプルウィジェット -----------------------------------------------------------------
 */
function canon_theme_options_tools_set_sample_widgets( $dp_options ) {
	...
	$sample_widgets = array(
		...
		array(
			...
			'params' => array(
        'post_num' => '3',
        'title1' => __('Recent post', 'tcd-canon'),
        'title2' => __('Recommend post', 'tcd-canon'),
        'content_type1' => 'post',
        'content_type2' => 'post',
        'post_order1' => 'date',
        'post_order2' => 'rand',
		...
		// 記事スライダー
		array(
			'class_name' => 'news_slider_widget',
			'params' => array(
        'title' => __('Recommend post', 'tcd-canon'),
        'post_num' => '3',
        'content_type' => 'post',
        'post_order' => 'date',
        'post_order2' => 'rand',
	...
	// ウィジェットエリアの設定
	foreach( array( 'footer_widget','common_widget' ) as $sidebar ) {
		// 現ウィジェットを設定を削除
 ...
 * canon テーマ初期化フィルター ----------------------------------------------------------------------------
 */
add_filter( 'tcd_theme_options_tools-get_default_images_settings', 'canon_theme_options_tools_get_default_images_settings', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'canon_theme_options_tools_set_sample_categories', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'canon_theme_options_tools_set_sample_posts', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'canon_theme_options_tools_set_sample_menus', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'canon_theme_options_tools_set_sample_widgets', 10, 1 );
