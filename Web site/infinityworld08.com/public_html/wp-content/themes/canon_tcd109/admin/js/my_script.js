jQuery(document).ready(function($){
  ...
  // ギャラリーカテゴリー
  $(document).on('click', '#gallery_use_category_yes', function(event){
    if ($(this).prop("checked")) {
      $('.gallery_non_category_option').hide();
      $('.gallery_category_option').show();
    }
  });
  $(document).on('click', '#gallery_use_category_no', function(event){
    if ($(this).prop("checked")) {
      $('.gallery_non_category_option').show();
      $('.gallery_category_option').hide();
    }
  });
  if ($('#gallery_use_category_yes').prop("checked")) {
    $('.gallery_non_category_option').hide();
    $('.gallery_category_option').show();
  }
  if ($('#gallery_use_category_no').prop("checked")) {
    $('.gallery_non_category_option').show();
    $('.gallery_category_option').hide();
  }
  ...
  // ブログ日付
  $(document).on('click', '#blog_show_date_display', function(event){
    if ($(this).prop("checked")) {
      $('.blog_show_update_option').show();
    }
  });
  $(document).on('click', '#blog_show_date_hide', function(event){
    if ($(this).prop("checked")) {
      $('.blog_show_update_option').hide();
    }
  });
  if ($('#blog_show_date_display').prop("checked")) {
    $('.blog_show_update_option').show();
  }
  if ($('#blog_show_date_hide').prop("checked")) {
    $('.blog_show_update_option').hide();
  }
  ...
  // トップページのヘッダー　キャプションのタイプ
  $(document).on('click', '#index_header_caption_type1', function(event){
    if ($(this).prop("checked")) {
      $('#index_header_caption_type1_option').show();
      $('#index_header_caption_type2_option').hide();
    }
  });
  $(document).on('click', '#index_header_caption_type2', function(event){
    if ($(this).prop("checked")) {
      $('#index_header_caption_type1_option').hide();
      $('#index_header_caption_type2_option').show();
    }
  });
  if ($('#index_header_caption_type1').prop("checked")) {
    $('#index_header_caption_type1_option').show();
    $('#index_header_caption_type2_option').hide();
  }
  if ($('#index_header_caption_type2').prop("checked")) {
    $('#index_header_caption_type1_option').hide();
    $('#index_header_caption_type2_option').show();
  }
  ...
  // フッター
  $(document).on('click', '#footer_widget_bg_type1', function(event){
    if ($(this).prop("checked")) {
      $('.footer_bg_type1_option').show();
      $('.footer_bg_type2_option').hide();
    }
  });
  $(document).on('click', '#footer_widget_bg_type2', function(event){
    if ($(this).prop("checked")) {
      $('.footer_bg_type1_option').hide();
      $('.footer_bg_type2_option').show();
    }
  });
  if ($('#footer_widget_bg_type1').prop("checked")) {
    $('.footer_bg_type1_option').show();
    $('.footer_bg_type2_option').hide();
  }
  if ($('#footer_widget_bg_type2').prop("checked")) {
    $('.footer_bg_type1_option').hide();
    $('.footer_bg_type2_option').show();
  }
  ...
  $(document).on('change', 'input.show_category_list', function(){
    var radioval = $(this).val();
    if(radioval == 'hide'){
      $(this).closest('.option_list').find('.show_category_list_hide_option').show();
      $(this).closest('.option_list').find('select.post_list_type').each(function(){
      ...
      $(this).closest('.option_list').find('.show_category_list_hide_option').hide();
      $(this).closest('.option_list').find('.show_category_list_display_option').show();
    }
  });

  // 関連記事
  $('select.post_list_type.news').each(function(){
    if ( $(this).val() == 'all_post' || $(this).val() == 'recommend_post' || $(this).val() == 'recommend_post2' || $(this).val() == 'recommend_post3') {
      $(this).closest('.option_list').find('.post_list_type_normal_option').show();
      $(this).closest('.option_list').find('.post_list_type_custom_option').hide();
      $(this).closest('.option_list').find('.post_list_type_category_option').hide();
  ...
  $(document).on('change', '#header_catch_layout', function(){
    $('#page_catch_layout_image img').removeClass('active');
    if ($(this).val() == 'type1') {
      $('#page_catch_layout_image .type1').addClass('active');
    } else if ($(this).val() == 'type2') {
      $('#page_catch_layout_image .type2').addClass('active');
    } else if ($(this).val() == 'type3') {
      $('#page_catch_layout_image .type3').addClass('active');
    } else if ($(this).val() == 'type4') {
      $('#page_catch_layout_image .type4').addClass('active');
    } else if ($(this).val() == 'type5') {
      $('#page_catch_layout_image .type5').addClass('active');
    } else if ($(this).val() == 'type6') {
      $('#page_catch_layout_image .type6').addClass('active');
    }
  });
  ...
  $(document).on('click', '.index_header_content_type1', function(event){
    ...
    $('.non_index_header_content_type1_option').hide();
    $('#index_header_content_height_option').removeClass('two_button');
    $("#index_header_content_height_type3").next().removeClass('non_active');
  });
  $(document).on('click', '.index_header_content_type2', function(event){
    ...
    $('.non_index_header_content_type1_option').show();
    if ($('#index_header_content_height_type3').prop("checked")) {
      $("#index_header_content_height_type1").trigger("click");
    }
    $('#index_header_content_height_option').addClass('two_button');
    $("#index_header_content_height_type3").next().addClass('non_active');
    $('.index_header_content_height_type3_option').hide();
  });
  ...
  // トップページ　スライダーの高さ
  $(document).on('click', '#index_header_content_height_type1', function(event){
    $('.index_header_content_height_type3_option').hide();
  });
  $(document).on('click', '#index_header_content_height_type2', function(event){
    $('.index_header_content_height_type3_option').hide();
  });
  $(document).on('click', '#index_header_content_height_type3', function(event){
    $('.index_header_content_height_type3_option').show();
  });

  if ($('#index_header_content_height_type1').is(":checked")) {
    $('.index_header_content_height_type3_option').hide();
  }
  if ($('#index_header_content_height_type2').is(":checked")) {
    $('.index_header_content_height_type3_option').hide();
  }
  if ($('#index_header_content_height_type3').is(":checked")) {
    $('.index_header_content_height_type3_option').show();
  }
  ...
  $(document).on('click', '.sub_box_tab .tab', function(event){
    ...
    $(this).closest('.tab_parent').children('.sub_box_tab_content').each( function() {
...
//    $(this).closest('.tab_parent').find('[data-tab-content="'+tab_name+'"]').addClass('active').siblings().removeClass('active');
  ...
  // 固定ページ　LP　ヘッダーの高さ
  $(document).on('click', '#page_lp_header_type1', function(event){
    if ($(this).prop("checked")) {
      $('.lp_header_type1_option').show();
      $('.lp_header_type2_option').hide();
    }
  });
  $(document).on('click', '#page_lp_header_type2', function(event){
    if ($(this).prop("checked")) {
      $('.lp_header_type1_option').hide();
      $('.lp_header_type2_option').show();
    }
  });
  if ($('#page_lp_header_type1').prop("checked")) {
    $('.lp_header_type1_option').show();
    $('.lp_header_type2_option').hide();
  }
  if ($('#page_lp_header_type2').prop("checked")) {
    $('.lp_header_type1_option').hide();
    $('.lp_header_type2_option').show();
  }
  ...
  // 固定ページのヘッダー入力欄を隠す
  $(document).on('click', '#hide_page_header_image_no', function(event){
    if ($(this).prop("checked")) {
      $('#page_header_setting_area').show();
    }
  });
  $(document).on('click', '#hide_page_header_image_yes', function(event){
    if ($(this).prop("checked")) {
  ...
  $(document).on('click', '#hide_sidebar_left', function(event){
    $('#page_design_content_option').hide();
    $('#page_access_option').hide();
  });
  ...
  $(document).on('click', '#hide_sidebar_hide', function(event){
    $('#page_design_content_option').show();
    $('#page_access_option').show();
  });


  // 固定ページ（ヘッダーメッセージ）
  $(document).on('click', '#hide_header_message_no', function(event){
    if ($(this).prop("checked")) {
      $('.header_message_option').show();
    }
  });
  $(document).on('click', '#hide_header_message_yes', function(event){
    if ($(this).prop("checked")) {
      $('.header_message_option').hide();
    }
  });
  if ($('#hide_header_message_no').prop("checked")) {
    $('.header_message_option').show();
  }
  if ($('#hide_header_message_yes').prop("checked")) {
    $('.header_message_option').hide();
  }


  // 固定ページ（プラン一覧）
  $(document).on('click', '#hide_plan_list_no', function(event){
    if ($(this).prop("checked")) {
      $('#page_plan_list_option').show();
    }
  });
  $(document).on('click', '#hide_plan_list_yes', function(event){
    if ($(this).prop("checked")) {
      $('#page_plan_list_option').hide();
    }
  });
  if ($('#hide_plan_list_no').prop("checked")) {
    $('#page_plan_list_option').show();
  }
  if ($('#hide_plan_list_yes').prop("checked")) {
    $('#page_plan_list_option').hide();
  }
  ...
  $(document).on('click', '#hide_sidebar_hide', function(event){
    if ($(this).prop("checked")) {
      $('#page_image_slider_setting_area').show();
    }
  });
  $(document).on('click', '#hide_sidebar_left, #hide_sidebar_right', function(event){
    if ($(this).prop("checked")) {
      $('#page_image_slider_setting_area').hide();
    }
  });
  if ($('#hide_sidebar_hide').prop("checked")) {
    $('#page_image_slider_setting_area').show();
  }
  if ($('#hide_sidebar_left, #hide_sidebar_right').prop("checked")) {
    $('#page_image_slider_setting_area').hide();
  }


  // LP ヘッダー スタイル
  $(document).on('click', '#header_style_type1', function(event){
    if ($(this).prop("checked")) {
      $('.header_height_option').show();
      $('.header_height_option').next().removeClass('hide_border');
      $('.header_style1_option').show();
      $('.header_style2_option').hide();
      $('.header_style3_option').hide();
      $('#page_image_slider_setting_area').hide();
    }
  });
  $(document).on('click', '#header_style_type2', function(event){
    if ($(this).prop("checked")) {
      $('.header_height_option').show();
      $('.header_height_option').next().removeClass('hide_border');
      $('.header_style1_option').hide();
      $('.header_style2_option').show();
      $('.header_style3_option').hide();
      $('#page_image_slider_setting_area').show();
    }
  });
  $(document).on('click', '#header_style_type3', function(event){
    if ($(this).prop("checked")) {
      $('.header_height_option').hide();
      $('.header_height_option').next().addClass('hide_border');
      $('.header_style1_option').hide();
      $('.header_style2_option').hide();
      $('.header_style3_option').show();
      $('#page_image_slider_setting_area').show();
    }
  });
  ...
  function lp_page_template() {
    $('.hide_border_option').removeClass('hide_border');
    $('.normal_template_option').hide();
    $('.normal_template_option_fix').removeClass('active');
    $('.space_fix').addClass('space');
    ...
    if ($('#page_lp_header_type2').is(':checked')) {
      $("#page_lp_header_type2").trigger("click");
    } else {
      $("#page_lp_header_type1").trigger("click");
    }
    if ($('#hide_page_header_image_yes').prop("checked")) {
      $('#page_header_setting_area').hide();
    }
    if ($('#hide_page_header_image_no').prop("checked")) {
      $('#page_header_setting_area').show();
    }
    if ($('#hide_header_message_no').prop("checked")) {
      $('.header_message_option').show();
    ...
    $('#page_image_slider_setting_area').show();
    if ($('#header_style_type1').prop("checked")) {
      $("#header_style_type1").trigger("click");
    }
    if ($('#header_style_type2').prop("checked")) {
      $("#header_style_type2").trigger("click");
    }
    if ($('#header_style_type3').prop("checked")) {
      $("#header_style_type3").trigger("click");
    }
  }
  function normal_template() {
    $('.hide_border_option').addClass('hide_border');
    $('.normal_template_option').show();
    $('.normal_template_option_fix').addClass('active');
    $('.space_fix').removeClass('space');
    ...
    $('.editor-block-list__layout').show();
    $('.edit-post-visual-editor .block-editor-block-list__layout').show();
    if ($('#hide_plan_list_yes').is(':checked')) {
      $("#page_plan_list_option").hide();
    } else {
      $("#page_plan_list_option").show();
    }
  ...
  var init_repeater = function(el) {
    $(el).each(function() {
      ...
      $repeater_wrapper.find(".sortable").sortable({
        ...
        handle: '> .theme_option_subbox_headline, > .price_data > .price_data_handler',
        //helper: "clone",
        start: function(e, ui){
          ui.item.find('textarea').each(function () {
            if (window.tinymce) {
  ...
  // 固定ページ（デザインコンテンツ）
  if ($('#design_content_list_type1').prop("checked")) {
      $('.design_content_list_type1_option').show();
      $('.design_content_list_type2_option').hide();
  }
  if ($('#design_content_list_type2').prop("checked")) {
      $('.design_content_list_type1_option').hide();
      $('.design_content_list_type2_option').show();
  }
...
*/
  $(document).on('change keyup', '.js-contents-builder-item-label', function(){
    $(this).closest('.js-contents-builder-item').find('.js-contents-builder-item-label-target').text($(this).val());
  });
  $('.js-contents-builder-item-label').each(function(){
    $(this).closest('.js-contents-builder-item').find('.js-contents-builder-item-label-target').text($(this).val());
  });
