...
function seo_meta_box() {
  ...
  $post_types = array ( 'post', 'page', 'news', 'service', 'gallery');
  add_meta_box(
    'show_seo_meta_box',//ID of meta box
    __('SEO', 'tcd-canon'),//label
...
<div class="tcd_custom_fields">
 ...
 <div class="tcd_cf_content">
  <h3 class="tcd_cf_headline"><?php _e( 'Title tag', 'tcd-canon' ); ?></h3>
  <input type="text" name="tcd-w_meta_title" value="<?php if(!empty($seo_title)){ echo esc_attr($seo_title); }; ?>" style="width:100%" />
 </div><!-- END .content -->

 <div class="tcd_cf_content">
  <h3 class="tcd_cf_headline"><?php _e( 'Meta description tag', 'tcd-canon' ); ?></h3>
  <div class="theme_option_message2" style="margin-top:10px;">
   <p><?php printf(__('Recommended number of characters is %s.', 'tcd-canon'), '180'); ?></p>
  </div>
  ...
  <p class="word_count_result"><?php _e( 'Current character is : <span>0</span>', 'tcd-canon' ); ?></p>
 </div><!-- END .content -->
...
function change_title_tag($title) {
  ...
  } elseif ( is_page_template('page-tcd-lp.php') && get_post_meta($post->ID, 'lp_header_type', true) == 'type2' && get_post_meta( $post->ID, 'lp_header_title', true ) ) {

    $title = get_post_meta( $post->ID, 'lp_header_title', true ) .  ' | ' . $site_title;
  ...
  // お知らせカテゴリーページ
    ...
    $seo_title = !empty($term_meta['seo_title']) ? $term_meta['seo_title'] : '';
    $category_title = $query_obj->name;

    if($seo_title){
      $title = $seo_title;
    } else {
      $title = $category_title .  ' | ' . $site_title;
    }
  ...
  // 検索結果ページ
    ...
    if ( !empty( get_search_query() ) ) {
      $title = sprintf( __( 'Search result for %s', 'tcd-canon' ), get_search_query() ) . ' | ' . $site_title;
    } else {
      $title = __( 'Search result', 'tcd-canon' ) . ' | ' . $site_title;
    }

  // 日月アーカイブ
  } elseif ( is_day() ) {

    $title = sprintf( __( 'Archive for %s', 'tcd-canon' ), get_the_time( __( 'F jS, Y', 'tcd-canon' ) ) ) . ' | ' . $site_title;

  } elseif ( is_month() ) {

    $title = sprintf( __( 'Archive for %s', 'tcd-canon' ), get_the_time( __( 'F, Y', 'tcd-canon') ) ) . ' | ' . $site_title;

  } elseif ( is_year() ) {

    $title = sprintf( __( 'Archive for %s', 'tcd-canon' ), get_the_time( __( 'Y', 'tcd-canon') ) ) . ' | ' . $site_title;
  ...
  // 著者ページ
    ...
    $author_name = sprintf( __( '%s\'s post list', 'tcd-canon' ), $curauth->display_name );
...
function get_seo_description() {
  ...
  // お知らせカテゴリーページ
    ...
    $seo_description = !empty($term_meta['seo_description']) ? $term_meta['seo_description'] : '';

    if($seo_description){
      return esc_html($seo_description);
    } else {
      return esc_html( $site_description );
    }
  ...
  // カテゴリー、タグ、タクソノミー
    ...
    return sprintf( __( 'Archive for %s', 'tcd-canon' ), get_the_time( __( 'F jS, Y', 'tcd-canon' ) ) );

  } elseif ( is_month() ) {

    return sprintf( __( 'Archive for %s', 'tcd-canon' ), get_the_time( __( 'F, Y', 'tcd-canon' ) ) );

  } elseif ( is_year() ) {

    return sprintf( __( 'Archive for %s', 'tcd-canon' ), get_the_time( __( 'Y', 'tcd-canon' ) ) );

  } elseif ( is_author() ) {

    global $wp_query;
    $curauth = $wp_query->get_queried_object();
    return sprintf( __( 'Archive for %s', 'tcd-canon' ), esc_html( $curauth->display_name ) );

  } elseif ( is_search() ) {

    return sprintf( __( 'Post list for %s', 'tcd-canon' ), get_search_query() );
