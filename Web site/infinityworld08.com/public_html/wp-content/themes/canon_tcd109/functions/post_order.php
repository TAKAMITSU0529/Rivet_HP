...
function po_admin_print_scripts() {
	...
	if ('page' === $post_type || 'gallery' === $post_type || 'service' === $post_type) {
...
function change_post_order() {
	...
	if ( 'page' !== $post_type && 'gallery' !== $post_type && 'service' !== $post_type) return;
...
function po_pre_get_posts( $query ) {
  ...
  if ( (is_tax('gallery_category') && $query->is_main_query()) ) { 
    $query->set( 'orderby', array( 'menu_order' => 'ASC', 'date' => 'DESC' ) );
  }
...
function po_get_previous_post_where( $where, $in_same_term, $excluded_terms, $taxonomy, $post ) {
	if ( in_array( $post->post_type, ['gallery'] ) ) {
