...
function pw_meta_box() {
	...
	add_meta_box(
		...
		__( 'Contents to encourage member registration', 'tcd-canon' ), // label
...
<div class="tcd_custom_fields">
	<div class="tcd_cf_content">
		<select name="pw_content">
			<option value=""><?php _e( 'Select a content to encourage member registration.', 'tcd-canon' ); ?></option>
			<?php for ( $i = 1; $i <= 3; $i++ ) : ?>
...
function tcd_password_form() {
	...
	if ( ! $pw_content ) {
		...
		$output .= '<input class="c-pw__box-input" name="post_password" id="' . $label . '" type="password" placeholder="'.__( 'Password:', 'tcd-canon' ).'" size="20"><input class="c-pw__btn c-pw__btn--submit" type="submit" name="Submit" value="' . esc_attr( __( 'Submit', 'tcd-canon' ) ) . '"></div>';

	} else {

		$output .= '<div class="c-pw__box-inner">';

		$pw_content_label = ( $dp_options['pw_label' . $pw_content] ) ? esc_html( $dp_options['pw_label' . $pw_content] ) : __( 'Password', 'tcd-canon' );
		$pw_password_button_label = ( $dp_options['pw_password_button_label' . $pw_content] ) ? esc_attr( $dp_options['pw_password_button_label' . $pw_content] ) : __( 'Submit', 'tcd-canon' );
	...
	if ( isset ( $_COOKIE[ 'wp-postpass_' . COOKIEHASH ] ) && wp_get_raw_referer() === get_permalink() ) {
		$output .= '<p class="c-pw__box-error">' . __( 'Password is incorrect.', 'tcd-canon' ) . '</p>';
	}
