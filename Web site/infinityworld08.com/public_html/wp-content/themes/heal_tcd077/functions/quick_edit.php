...
function custom_quick_edit( $column_name, $post_type ) {
  ...
  if ( $print_nonce ) {
    ...
    // seo
    if ( in_array( $post_type, array( 'post', 'page', 'recipe', 'news' ) ) ) {
...
<?php
    ...
    if ( $post_type == 'post' ) {
      ...
      echo '<input type="hidden" name="blog_custom_fields_meta_box_nonce2" value="', wp_create_nonce( 'blog_premium.php' ), '" />';
?>
<fieldset class="inline-edit-col-right">
  <div class="inline-edit-col column-recommend">
    <div class="inline-edit-group">
      ...
      <label style="line-height:1.5;"><input type="checkbox" name="premium_post" value="1" /> <?php _e( 'Show this post for premium post', 'tcd-w' );  ?></label>
    </div>
  </div>
</fieldset>
<?php
...
function custom_quick_edit_values( $actions, $post ) {
    ...
    if ( $post->post_type == 'post' ) {
		$meta_keys = array_merge( $meta_keys,  array( 'recommend_post', 'recommend_post2', 'pickup_post', 'premium_post' ) );
	}
