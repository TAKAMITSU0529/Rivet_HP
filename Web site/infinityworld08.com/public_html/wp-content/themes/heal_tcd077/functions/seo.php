...
function seo_meta_box() {
  $post_types = array ( 'post', 'page', 'news', 'campaign', 'menu');
  add_meta_box(
...
function seo_title( $title, $sep ) {
  ...
  $options = get_design_plus_option();
	...
	if ( is_single() && get_post_meta( $post->ID, 'tcd-w_meta_title', true ) or is_page() && get_post_meta( $post->ID, 'tcd-w_meta_title', true ) ) {
  	...
  	$title = sprintf( __( 'Post list for %s', 'tcd-w' ), $options['shop_label'] );
 	} elseif ( is_category() ) {
...
function get_seo_description() {
	...
	} elseif ( ( is_single() && !is_singular('menu') && !is_singular('staff') ) || is_page() ) {
