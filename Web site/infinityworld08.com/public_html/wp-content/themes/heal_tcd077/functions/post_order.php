...
function po_admin_print_scripts() {
	...
	if ( 'menu' === $post_type || 'voice' === $post_type || 'staff' === $post_type) {
...
/**
 * Change post order of custom post type 'faq' and 'clinic'
 */
function change_post_order() {
	...
	if ( 'menu' !== $post_type && 'voice' !== $post_type && 'staff' !== $post_type) return;
...
function po_pre_get_posts( $query ) { 
  ...
  if ( $query->is_post_type_archive( 'menu' ) || $query->is_post_type_archive( 'voice' ) || $query->is_post_type_archive( 'staff' )) { 
