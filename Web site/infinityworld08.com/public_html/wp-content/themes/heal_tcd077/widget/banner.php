...
<a class="link animate_background num<?php echo $i; ?>" href="<?php if($url) { echo esc_url($url); }; ?>"<?php if($target) { echo ' target="_blank"'; }; ?>>
 ...
 <div class="title_area" style="color:<?php if($font_color) { echo esc_attr($font_color); }; ?>; background:rgba(<?php echo esc_attr($overlay_color); ?>,<?php echo esc_attr($overlay_opacity); ?>); ">
  <?php if($title) { ?>
  <p class="title rich_font_<?php if($font_type) { echo esc_attr($font_type); }; ?>" style="font-size:<?php if($title_font_size) { echo esc_attr($title_font_size); }; ?>px;"><?php echo nl2br(esc_html($title)); ?><?php if($sub_title) { ?><span style="font-size:<?php if($sub_title_font_size) { echo esc_attr($sub_title_font_size); }; ?>px;"><?php echo nl2br(esc_html($sub_title)); ?></span><?php }; ?></p>
  <?php }; ?>
 </div>
 <?php }; ?>
...
<?php
  ...
  function form($instance) {
    for ( $i = 1; $i <= 4; $i++ ) {
      ...
      $defaults['banner_title_font_type'.$i] = 2;
    }
...
<div class="tcd_ad_widget_box">
  ...
  <div class="tcd_widget_content">
   ...
   <?php echo UI\font_select( $this->get_field_name( 'banner_title_font_type' . $i ), $instance[ 'banner_title_font_type' . $i ] ); ?>
  </div>
  ...
  <div class="tcd_widget_content">
   <h3 class="tcd_widget_headline"><?php _e('Overlay', 'tcd-w'); ?></h3>
