...
function tcd_font_size_option($dp_options, $key, $repeater = false) {
  ...
  if($repeater == false){
    $name = 'dp_options['.$key.']';
    $name_sp = 'dp_options['.$key.'_mobile]';
    $value = esc_attr( $dp_options[$key] );
    $value_sp = esc_attr( $dp_options[$key.'_mobile'] );
  }else{
    $name = 'dp_options['.$key[0].']['.$key[1].']['.$key[2].']';
    $name_sp = 'dp_options['.$key[0].']['.$key[1].']['.$key[2].'_mobile]';
    $value = esc_attr( $dp_options[$key[2]] );
    $value_sp = esc_attr( $dp_options[$key[2].'_mobile'] );
  }
...
function cf_tcd_font_size_option($key, $variable, $variable_sp) {
  ...
  // SP
  $html .= '<label class="font_size_label number_option">';
  $html .= '<input class="hankaku input_font_size_sp" type="number" name="'.$key.'_mobile" value="'.esc_attr( $variable_sp ).'" />';
  $html .= '<span class="icon icon_sp"></span>';
  $html .= '</label>';
