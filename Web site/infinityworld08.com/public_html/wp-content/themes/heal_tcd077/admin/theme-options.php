...
require get_template_directory() . '/admin/inc/menu.php';
require get_template_directory() . '/admin/inc/campaign.php';
require get_template_directory() . '/admin/inc/staff.php';
require get_template_directory() . '/admin/inc/voice.php';
...
/**
 * HEAL オプションTools デフォルト画像設定フィルター
 */
function heal_theme_options_tools_get_default_images_settings( $default_images_settings ) {
	// デフォルト画像設定
	return array(
/*
		array(
...
*/
		...
		array(
			...
			'theme_option_keys' => array( 'index_slider_image1', 'index_slider_image2', 'index_slider_image3')
		),
		array(
			'media_filename' => 'image_480x300.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_480x300.gif',
			'theme_option_keys' => array( 'index_box_content_image1','index_box_content_image2','index_box_content_image3','footer_banner_image1', 'footer_banner_image2', 'footer_banner_image3')
		),
		array(
			'media_filename' => 'image_333x300.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_333x300.gif',
			'theme_option_keys' => array( "['contents_builder'][0]['dc1_image1']", "['contents_builder'][0]['dc1_image2']","['contents_builder'][0]['dc1_image3']" )
		),
		array(
			'media_filename' => 'image_1000x300.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_1000x300.gif',
			'theme_option_keys' => array( "['contents_builder'][1]['dc2_banner_image']" )
		),
		array(
			'media_filename' => 'image_1450x616.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_1450x616.gif',
			'theme_option_keys' => array( "['contents_builder'][3]['campaign_image']" )
		),
		array(
			'media_filename' => 'image_1450x430.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_1450x430.gif',
			'theme_option_keys' => array( 'blog_bg_image', 'campaign_bg_image', 'menu_bg_image' , 'news_bg_image', 'staff_bg_image' , 'voice_bg_image')
		)
	);
}
...
/**
 * heal オプションTools サンプルカテゴリー
 */
function heal_theme_options_tools_set_sample_categories( $dp_options ) {
	// 新規サイト以外、リセットチェックなしの場合は終了
	if ( ! theme_options_tools_is_new_site() && empty( $_POST['tcd-tools-reset-sample-categories'] ) )
		return $dp_options;

	// キャンペーンカテゴリーのアイキャッチ画像
	$campaign_category_media_id = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'image_810x455.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_810x455.gif'
	), true );
...
/**
 * heal オプションTools サンプル記事 ---------------------------------------------------------------------------
 */
function heal_theme_options_tools_set_sample_posts( $dp_options ) {

	// ブログのアイキャッチ画像（キャンペーン、お知らせも同じ）
	$media_id = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'image_650x410.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_810x455.gif'
	), true );

	// メニューのアイキャッチ画像
	$menu_post_media_id = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'image_1450x430.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_1450x430.gif'
	), true );
	$menu_post_media_id2 = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'image_500x300.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_500x300.gif'
	), true );

	// スタッフのアイキャッチ画像（お客様の声も同じ）
	$staff_post_media_id = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'image_500x500.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_500x500.gif'
	), true );
	...
	for ( $i = 1; $i <= 6; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample campaign', 'tcd-w' ) . $i,
			'post_content' => __( 'Sample text. Sample text.', 'tcd-w' ),
			'post_name' => 'sample-campaign' . $i,
			'post_status' => 'publish',
			'post_type' => 'campaign',
			'post_date' => $post_date_base . sprintf( '%02d', $i ),
			'thumbnail' => $media_id,
	...
	for ( $i = 1; $i <= 6; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample menu', 'tcd-w' ) . $i,
			'post_content' => __( 'Sample text. Sample text.', 'tcd-w' ),
			'post_name' => 'sample-menu' . $i,
			'post_status' => 'publish',
			'post_type' => 'menu',
			'post_date' => $post_date_base . sprintf( '%02d', $i ),
			'thumbnail' => $menu_post_media_id,
			'metas' => array(
				'menu_archive_image' => $menu_post_media_id2,
				'menu_header_title' => __( 'Title', 'tcd-w' ),
				'menu_header_sub_title' => __( 'Subtitle', 'tcd-w' ),
				'menu_header_desc' => __( 'Description will be displayed here.<br />Description will be displayed here.', 'tcd-w' )
			)
		);
	}
...
/**
 * heal オプションTools サンプルメニュー ------------------------------------------------------------------
 */
function heal_theme_options_tools_set_sample_menus( $dp_options ) {
...
/**
 * heal オプションTools サンプルウィジェット -----------------------------------------------------------------
 */
function heal_theme_options_tools_set_sample_widgets( $dp_options ) {
 ...
 * heal テーマ初期化フィルター ----------------------------------------------------------------------------
 */
add_filter( 'tcd_theme_options_tools-get_default_images_settings', 'heal_theme_options_tools_get_default_images_settings', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'heal_theme_options_tools_set_sample_categories', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'heal_theme_options_tools_set_sample_posts', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'heal_theme_options_tools_set_sample_menus', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'heal_theme_options_tools_set_sample_widgets', 10, 1 );
